...
def terrain_value_iteration(np.ndarray [DTYPE_t, ndim=2] terrain_mtx, np.ndarray [DTYPE_t, ndim=1] source, np.ndarray [DTYPE_t, ndim=1] target, np.ndarray [DTYPE_t, ndim=1] steps, int max_horizon, int append):
    ...
    cdef int[:, :] J_arr = J
    cdef int[:, :] Jprev_arr = Jprev
    cdef int *J_ptr = &J_arr[0, 0]
    cdef int *Jprev_ptr = &Jprev_arr[0, 0]

    # Policy matrix
    #policy = np.zeros((nX, nY))
    ...
    cdef int[:, :] descendantX_arr = descendantX
    cdef int[:, :] descendantY_arr = descendantY
    ...
    cdef Py_ssize_t xMin, yMin
    cdef Py_ssize_t xNext, yNext
 
    cdef Py_ssize_t k, x, y, uIdx
    ...
    trajs[0, :] = [source[0], source[1]]

    for k in range(max_horizon):
        trajs[k + 1, :] = [descendantX[trajs[k, 0], trajs[k, 1]], descendantY[trajs[k, 0], trajs[k, 1]]]
                
        if (abs(trajs[k+1, 0]-target[0]) + abs(trajs[k+1, 1]-target[1])) < 1:
            break
